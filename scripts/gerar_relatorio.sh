...
if [ ! -f "$ficheiro_dados" ]; then
    echo "O ficheiro $ficheiro_dados não foi encontrado no servidor."
    exit 1
fi

#nº linhas do ficheiro de dados
total_linhas=$(wc -l < "$ficheiro_dados")

#nº registos do ficheiro de dados= nº de linhas menos a linha de cabeçalho
